...
Every string in v1 was an English literal sitting in the branch that printed it, and "add language support" was one of the TODOs at the bottom of the file that never happened. Retrofitting locales is not hard, but it is invasive in a way that never feels urgent, so it does not get done. That is the argument for paying for it while there is almost no copy to convert.

The counter-argument is that a translation layer over four Sections is ceremony for its own sake, and the halfway option, routing every string through a lookup that only ever has one bundle behind it, pays the cost without buying anything.
