...
- `bin` and `main` in `package.json` point at `dist/index.js`, and `files` must keep `dist/` in the tarball. A publish that ships without `dist/` produces a package whose entry point does not exist — nothing in the repository can catch that, since the tree is expected not to contain it.
