...
`content`, `i18n` and `presentation` are a **functional core**: no `node:fs`, no `fetch`, no clock, no prompt. `infrastructure` is the **imperative shell** and owns every side effect the CLI has — retrieving the CV from its Source and writing it into the Downloads Folder. `cli.ts` sequences them and is the only module allowed to know about both.
...
- The shape has room to grow: a Section that needs data of its own adds a file to `content/`, not a folder. If a fifth folder ever becomes necessary, that is worth its own ADR — this one has been outgrown at that point.
