...
What was decided, in the present tense, as a rule the codebase follows: "the CV is fetched at Download time", not "we decided to fetch the CV". Name the alternative that was rejected and why, since that is what stops it being re-proposed. Point at the code with symbol names rather than line numbers, because a `file.ts:123` citation rots the moment anything above it moves.
...
- Where the decision bites in the rest of the docs: the [`CONTEXT.md`](../../CONTEXT.md) entry, the conventions in [`CLAUDE.md`](../../CLAUDE.md), the README section that has to link back here. An ADR that only the index links to will not be read.
