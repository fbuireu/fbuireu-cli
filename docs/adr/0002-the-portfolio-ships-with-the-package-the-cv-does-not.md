...
- The CV Source is load-bearing and is not an implementation detail: renaming or moving `assets/pdf/Ferran-Buireu-CV.pdf` in the `fbuireu/fbuireu` repository breaks every published version of this CLI at once, including versions that can no longer be patched. Nothing in CI can catch it, because the file lives in another repository.
