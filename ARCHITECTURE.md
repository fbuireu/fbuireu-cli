...
| # | Step | Layer | Notes |
| --- | --- | --- | --- |
| 1 | Parse `--lang`, read `LANG`/`LC_ALL` | entry | An unknown or unsupported value falls back to English rather than failing — [ADR 0004](./docs/adr/0004-the-locale-selects-the-cv-edition.md) |
...
| 7 | CV → verify the bytes are a PDF | infrastructure | A successful fetch is not the same as receiving the document — [ADR 0002](./docs/adr/0002-the-portfolio-ships-with-the-package-the-cv-does-not.md) |
...
- **Git hooks.** Husky: `pre-commit` → lint-staged (`biome check --write`), `commit-msg` → commitlint
  (conventional commits — the version number depends on them), `pre-push` → typecheck + changed tests + build.
- **Release.** `.releaserc.json`: semantic-release on `main` — commit-analyzer, release-notes-generator,
